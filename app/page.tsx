...
function Visual({ type }: { type: string }) {
  if (type === "history") return <div className="history">{[["/deck/image8.png","LIFEGUARDING · EAST HAMPTON"],["/deck/image2.png","STONY BROOK · LONG ISLAND"],["/deck/image3.jpeg","MIIS · MONTEREY"]].map(([src,label],i)=><figure key={src} style={{"--i":i} as React.CSSProperties}><img src={src} alt={label}/><figcaption>{label}</figcaption></figure>)}</div>;
  if (type === "timeline") return <div className="timeline">{[["JUNE 1","/deck/image3.png","Question box"],["JUNE 4","/deck/image4.png","Property first"],["JUNE 5","/deck/image5.png","Clear product"]].map(([d,src,label],i)=><figure key={d} style={{"--i":i} as React.CSSProperties}><b>{d}</b><img src={src} alt={`${d}: ${label}`}/><figcaption>{label}</figcaption></figure>)}</div>;
  if (type === "policy") return <img className="wide-shot" src="/deck/image6.png" alt="COL Policy Intelligence application screenshot"/>;
  if (type === "value") return <div className="diagrams"><button onClick={()=>window.open('/deck/image.jpeg','_blank')}><img src="/deck/image.jpeg" alt="Observing systems used to improve hurricane prediction diagram"/></button><button onClick={()=>window.open('/deck/image2.jpeg','_blank')}><img src="/deck/image2.jpeg" alt="U.S. Animal Telemetry Network value diagram"/></button></div>;
  if (type === "tools") return <div className="tool-flow"><div><small>EARLIER BUILD</small><strong>GitHub<br/>Streamlit<br/>Vercel<br/>Railway<br/>Supabase</strong></div><span>became →</span><strong>ChatGPT Sites<br/>&amp;<br/>Supabase</strong></div>;
  return null;
}
  ...
  return <main>
    <section className={`scene scene-${scene.type}`} aria-labelledby="scene-title">
      ...
      <div className="content" key={index}>
        ...
        {scene.type==='demo' && <div className="demo"><iframe ref={demoRef} src="https://ocean-evidence-commons.eliotfi.chatgpt.site/" title="Ocean Evidence Commons live demo"/><div className="demo-fallback"><img src="/deck/image7.png" alt="Static fallback preview of an IOOS evidence website"/><a href="https://ocean-evidence-commons.eliotfi.chatgpt.site/" target="_blank" rel="noreferrer">Open live demo ↗</a></div></div>}
      ...
      <footer><span>CENTER FOR OCEAN LEADERSHIP</span><img src="/deck/image.png" alt="UCP UCAR Community Programs"/></footer>
    </section>
